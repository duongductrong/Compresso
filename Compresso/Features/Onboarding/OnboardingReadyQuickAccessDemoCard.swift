...
struct OnboardingQuickAccessDemoCard: View {
    ...
    private var background: some View {
        ZStack {
            CompressoMaterialFill(shape: cardShape, kind: .ultraThin, fallbackOpacity: 0.72)
    ...
    private var topControls: some View {
        HStack {
            CompressoMaterialFill(shape: Circle(), kind: .regular, fallbackOpacity: 0.9)
            ...
            .compressoMaterialBackground(.regular, in: kindBadgeShape)
    ...
    private var placeholderContent: some View {
        VStack(alignment: .leading, spacing: 10) {
            Image(systemName: isTargeted ? "tray.full.fill" : "tray.and.arrow.down.fill")
                ...
                .compressoMaterialBackground(.regular, in: RoundedRectangle(cornerRadius: 10, style: .continuous))
