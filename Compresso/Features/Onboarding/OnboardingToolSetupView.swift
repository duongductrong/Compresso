...
struct OnboardingToolSetupView: View {
    ...
    private var statusSubtitle: String {
        if missingTools.isEmpty {
            ...
            return "Compresso will install only the missing Homebrew packages."
        } else {
            return "Homebrew is not installed, so Compresso cannot install dependencies automatically."
        }
    }

    private var dependencyParagraph: String {
        let names = OptimizationTool.catalog.map(\.name)
        return "Compresso uses \(formattedToolList(names)) to optimize media locally."
    }
