...
struct OnboardingReadyQuickAccessPreview: View {
    ...
    var body: some View {
        ...
        .compressoTask(id: reduceMotion) { await runPreviewLoop() }
    ...
    private var fileBundle: some View {
        ...
        .padding(8)
        .compressoMaterialBackground(.ultraThin, in: Capsule(), fallbackOpacity: 0.72)
...
private struct OnboardingReadyFileChip: View {
    ...
    var body: some View {
        ...
        .frame(height: 26)
        .compressoMaterialBackground(.regular, in: Capsule())
        .overlay(Capsule().strokeBorder(Color.primary.opacity(0.10), lineWidth: 1))
    }
}
