...
struct QuickAccessBoxView: View {
    ...
    private var boxBackground: some View {
        ZStack {
            ...
            CompressoMaterialFill(shape: boxShape, kind: .regular, fallbackOpacity: 0.86)
                .opacity(0.18)
    ...
    private var batchSizeComparisonText: String? {
        ...
        return "\(ByteCountFormatter.compressoString(fromByteCount: originalBytes)) -> \(ByteCountFormatter.compressoString(fromByteCount: optimizedBytes))"
    }
